...
public class SegmentedCellImages extends StructureImages {
    ...
    public synchronized ImageInt getImage(int structureIdx) {
        ...
        if (images[structureIdx] == null || !images[structureIdx].isOpened())
            images[structureIdx] = cell.getStructure(structureIdx).openSegmented();
    ...
    public synchronized ImageFloat getProbabilityMap(int structureIdx) {
        ...
        if (probabilityMap[structureIdx] == null || !probabilityMap[structureIdx].isOpened())
            probabilityMap[structureIdx] = ((Structure) cell.getStructure(structureIdx)).openProbabilityMap();
    ...
    public synchronized ImageFloat getDistanceMap(int structureIdx, int nbCPUs) {
        if (distanceMap[structureIdx] == null) {
            ...
            if (image != null)
                distanceMap[structureIdx] = EDT.run(image, 0f, (float) maskImage.getScaleXY(), (float) maskImage.getScaleZ(), structureIdx != 0, nbCPUs);
        }
