...
public abstract class Parameter implements MouseListener {
    ...
    protected Color setColor() {
        ...
        if (!isValidOrNotCompulsary()) {
            col = Color.red;
        } else if (template != null && !this.sameContent(template)) {
            col = Color.blue;
        }
    ...
    public String getHelp(boolean basic) {
        ...
        if ((!label2.endsWith(":")) && (!label2.endsWith(": "))) {
            label2 = label2.concat(": ");
        }
        String help = "<strong>" + label2 + "</strong>";
        if (basic) {
            if (this.helpBasic != null) {
                //ij.IJ.log(help + helpBasic);
                return help.concat(helpBasic);
            } else if (this.helpAdvanced != null) {
                return help.concat(helpAdvanced);
            } else {
                return help.concat("No help available.");
            }
        } else {
    ...
    public static void addToList(Parameter[] parameters, ArrayList<Parameter> list) {
        for (Parameter p : parameters) {
            if (p instanceof NestedParameter) {
                addToList(((NestedParameter) p).getParameters(), list);
            } else {
                list.add(p);
            }
        }
    ...
    protected static boolean sameValue(Parameter[] array1, Parameter[] array2) {
        if (array1 == null && array2 == null) {
            ...
            for (int i = 0; i < array1.length; i++) {
                if (!array1[i].sameContent(array2[i])) {
                    return false;
                }
            }
            return true;
