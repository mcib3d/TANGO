...
public abstract class SpotLocalThresholder {
    ...
    protected void localThreshold(double[] thlds, boolean rescueSpots) {
        ...
        //if (true) return;
        for (int i = 0; i < thlds.length; i++)
            localThreshold((Object3DVoxels) spots[i], (float) adjustThld(thlds[i]), rescueSpots);
        //if (debug) segMap.showDuplicate("after local threshold");
        if (rescueSpots) {
            if (debug) segMap.showDuplicate("before rescue");
            for (int i = 0; i < rescuedSpots.size(); i++)
                if (rescuedSpots.get(i).getVoxels().size() <= 2) {
    ...
    public void localThreshold(Object3DVoxels spot, float thld, boolean rescue) {
        ...
        if (rescue) {
            ...
            while (change) {
                ...
                for (Voxel3D v : voxels) {
                    int xy = v.getRoundX() + v.getRoundY() * sizeX;
                    if (segMap.getPixelInt(xy, v.getRoundZ()) == 0 && intensityMap.getPixel(xy, v.getRoundZ()) > max)
                        max = intensityMap.getPixel(xy, v.getRoundZ());
                }
                if (max > thld) { // && max>globalThld
                    ...
                    for (Voxel3D v : voxels) {
                        ...
                        if (segMap.getPixelInt(xy, v.getRoundZ()) == 0) {
                            ...
                            if (intensityMap.getPixel(xy, v.getRoundZ()) < min)
                                min = intensityMap.getPixel(xy, v.getRoundZ());
                        }
                    }
                    if (vox.size() > 1) {
