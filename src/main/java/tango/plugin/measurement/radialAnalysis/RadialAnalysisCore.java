...
public class RadialAnalysisCore {
    ...
    public RadialAnalysisCore(InputCellImages inputImages, SegmentedCellImages images, int structureIdx, int nbCPUs, boolean verbose) {
        ...
        if (verbose) {
            distanceMap.showDuplicate("distance map");
        }
    ...
    protected void initDistanceMap(ImageInt segmentedStructure, boolean inside) {
        ...
        if (verbose) {
            distanceMap.showDuplicate("radial analysis:distance map");
        }
    ...
    protected double getShell(float distance) {
        if (distance == 0 || sortedDistances.length == 0) {
            return 0;
        }
        int idx = Arrays.binarySearch(sortedDistances, distance);
        if (idx < 0) {
            return ((double) (-idx + 1) / (double) sortedDistances.length);
        }
        // case no structure --> constant image
        if (sortedDistances[0] == sortedDistances[sortedDistances.length - 1]) {
            return Double.NaN;
        } else {
           //recherche des bornes a gauche et à droite
            // on prend le milieu
            int left = idx;
            while (left > 0 && sortedDistances[left] == distance) {
                left--;
            }
            int right = idx;
            while (right < (sortedDistances.length - 1) && sortedDistances[right] == distance) {
                right++;
            }
    ...
    public double getMeanShell(Object3DVoxels o) {
        ...
        if (o.getVolumePixels() > 0) {
            d /= (double) o.getVolumePixels();
        }
