...
public class ObjectColocalization implements MeasurementObject2Object {
    ...
    public void getMeasure(InputCellImages raw, SegmentedCellImages seg, StructureQuantifications quantifs) {
        if (!overlap.isSelected() && !pc1.isSelected() && !pc2.isSelected()) {
            return;
        }
        if (structure1.getIndex() == structure2.getIndex()) {
            getMes(structure1.getObjects(seg), quantifs);
        } else {
            getMes(structure1.getObjects(seg), structure2.getObjects(seg), quantifs);
        }
    }
    ...
    private void getMes(Object3D[] objects1, Object3D[] objects2, StructureQuantifications quantifs) {
        ...
        double[] mespc2 = null;
        if (pc1.isSelected()) {
            mespc1 = new double[size1 * size2];
        }
        if (pc2.isSelected()) {
            mespc2 = new double[size1 * size2];
        }
        int offset = 0;
        for (Object3D o : objects1) {
            //ImageHandler label=o.getLabelImage();
...
//            IJ.log("offset "+ofX+" "+ofY+" "+ofZ);
    ...
    private void getMes(Object3D[] objects, StructureQuantifications quantifs) {
        ...
        double[] mespc2 = null;
        if (pc1.isSelected()) {
            mespc1 = new double[size];
        }
        if (pc2.isSelected()) {
            mespc2 = new double[size];
        }
        int count = 0;
        for (int i = 0; i < (objects.length - 1); i++) {
            ...
            for (int j = i + 1; j < objects.length; j++) {
                ...
                if (mespc1 != null) {
                    mespc1[count] = mes[count] / o.getVolumeUnit();
                }
                if (mespc2 != null) {
                    mespc2[count] = mes[count] / objects[j].getVolumeUnit();
                }
                count++;
            }
        }
